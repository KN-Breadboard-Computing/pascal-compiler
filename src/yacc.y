...
%token <ival> T_INT
%token <sval> T_IDENTIFIER T_CHAR T_STRING
%token T_BEGIN T_END
%token T_TRUE T_FALSE T_MAXINT
%token T_READ T_WRITE T_WRITELN T_CHR T_ORD T_PRED T_SUCC
...
program :
    T_PROGRAM T_IDENTIFIER T_SEMI routine T_DOT {
        $$ = $4;
        $$->setAttribute($2);
        ast = $$;
    }
|
    T_PROGRAM T_IDENTIFIER T_SEMI routine {
        $$ = $4;
        $$->setAttribute($2);
        ast = $$;
        errors.push_back("error at line " + to_string(linesCounter) + ", lack of end dot");
    }
|
    T_PROGRAM T_IDENTIFIER routine T_DOT {
        $$ = $3;
        $$->setAttribute($2);
        ast = $$;
        errors.push_back("error at line " + to_string(linesCounter) + ", lack of semicolon");
    }
|
    T_PROGRAM T_IDENTIFIER routine {
        $$ = $3;
        $$->setAttribute($2);
        ast = $$;
        errors.push_back("error at line " + to_string(linesCounter) + ", lack of semicolon and end dot");
    }
;
...
routine_head :
    const_part type_part var_part routine_part {
        $$ = new TreeDefine(ROUTINEHEAD, linesCounter);
        $$->addChild($1);
        $$->addChild($2);
        $$->addChild($3);
        $$->addChild($4);
...
function_decl :
    function_head T_SEMI routine T_SEMI {
        $$ = new TreeDefine(FUNCTION, linesCounter);
        $$->addChild($1);
        $$->addChild($3);
    }
|
    function_head T_SEMI routine {
        ...
        $$->addChild($1);
        $$->addChild($3);
        errMsg.push_back("error at line " + to_string(linesCounter) + ", lack of semicolon");
    }
|
...
function_head :
    T_FUNCTION T_IDENTIFIER parameters  T_COLON simple_type_decl {
        $$ = new TreeDefine(FUNCTIONHEAD, linesCounter);
        $$->setAttribute($2);
        $$->addChild($3);
        $$->addChild($5);
    }
...
procedure_decl :
    procedure_head T_SEMI routine T_SEMI {
        $$ = new TreeDefine(PROCEDURE, linesCounter);
        $$->addChild($1);
        $$->addChild($3);
    }
;
...
procedure_head :
    T_PROCEDURE T_IDENTIFIER parameters {
        $$ = new TreeDefine(PROCEDUREHEAD, linesCounter);
        $$->setAttribute($2);
        $$->addChild($3);
    }
;
...
para_type_list :
    T_VAR name_list T_COLON simple_type_decl {
        $$ = new TreeDefine(VAR_PARA, linesCounter);
        $$->addChild($2);
        $$->addChild($4);
    }
|
    name_list T_COLON simple_type_decl {
        $$ = new TreeDefine(VAL_PARA, linesCounter);
        $$->addChild($1);
        $$->addChild($3);
    }
;
...
var_decl :
    name_list T_COLON type_decl T_SEMI {
        $$ = new TreeDefine(VAR, linesCounter);
        $$->addChild($1);
        $$->addChild($3);
    }
|
...
const_expr :
    ID T_EQUAL const_value T_SEMI {
        $$ = new TreeDefine(DK_CONST, linesCounter);
...
const_value :
    T_INT {
        $$ = new TreeDefine(EK_CONST, linesCounter);
        $$->setExpType(INT);
        $$->setAttribute($1);
    }
|   T_CHAR
...
type_definition :
    ID T_EQUAL type_decl T_SEMI {
        $$ = new TreeDefine(TYPE, linesCounter);
        $$->addChild($1);
        $$->addChild($3);
    }
|
...
field_decl :
    name_list T_COLON type_decl T_SEMI {
        $$ = new TreeDefine(TK_RECORD, linesCounter);
        $$->addChild($1);
        $$->addChild($3);
    }
|
...
array_type_decl :
    T_ARRAY T_LB simple_type_decl T_RB T_OF type_decl {
        $$ = new TreeDefine(TK_ARRAY, linesCounter);
        $$->addChild($3);
        $$->addChild($6);
        $$->setExpType(ET_ARRAY);
    }
...
simple_type_decl :
    ID {
        $$ = new TreeDefine(SIMPLE_ID, linesCounter);
        $$->setAttribute($1->getAttribute());
        //free($1);
    }
|
    T_LP name_list T_RP {
        $$ = new TreeDefine(TK_SIMPLE_ENUM, linesCounter);
        $$->addChild($2);
        $$->setExpType(ET_SIMPLE_ENUM);
    }
|
    const_value T_DOTDOT const_value {
        $$ = new TreeDefine(TK_SIMPLE_LIMIT, linesCounter);
        $$->addChild($1);
        $$->addChild($3);
...
|
    T_INTEGER_TYPE {
        $$ = new TreeDefine(SIMPLE_SYS, linesCounter);
        $$->setExpType(INT);
    }
|
    T_BOOLEAN_TYPE {
...
ID :
    T_IDENTIFIER {
        $$ = new TreeDefine(EK_ID, linesCounter);
        $$->setAttribute($1);
    }
;
