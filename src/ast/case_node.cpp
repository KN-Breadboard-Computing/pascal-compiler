...
namespace ast {
void CaseNode::accept(AstVisitor *visitor) const {
  visitor->visit(this);
}
...
[[nodiscard]] std::unique_ptr<AstNode> CaseNode::clone() const {
  ...
  std::vector<std::pair<ExpressionNode *, StatementNode *> *>
	  *newStatements = new std::vector<std::pair<ExpressionNode *, StatementNode *> *>{};
  for (auto &statement : *statements_) {
	newStatements->push_back(new std::pair<ExpressionNode *, StatementNode *>{
		dynamic_cast<ExpressionNode *>(statement->first->clone().release()),
