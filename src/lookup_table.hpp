...
class LookupTable {
 public:
  ...
  LookupTable() : scopes_{}, types_{}, variables_{}, routines_{} {
    types_["integer"][""] = TypeInfo{TypeCategory::SIMPLE, "integer", "integer", true};
    types_["unsigned"][""] = TypeInfo{TypeCategory::SIMPLE, "unsigned", "unsigned", true};
...
#endif
    types_[name][currentScope] = TypeInfo{category, name, type, true};
  }
  ...
  [[nodiscard]] std::vector<TypeInfo> getTypes(std::function<bool(const std::string&, const TypeInfo&)> indicator) const {
...
#endif
    routines_[name][currentScope] = RoutineInfo{category, name, args, type, true};
  }
